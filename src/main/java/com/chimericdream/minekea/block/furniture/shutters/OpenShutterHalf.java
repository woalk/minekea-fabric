...
public class OpenShutterHalf extends Block implements MinekeaBlock, Waterloggable {
    ...
    public void onBreak(World world, BlockPos pos, BlockState state, PlayerEntity player) {
        ...
        if (centerState.get(WATERLOGGED)) {
            world.createAndScheduleFluidTick(centerPos, Fluids.WATER, Fluids.WATER.getTickRate(world));
        }

        super.onBreak(world, centerPos, centerState, player);
    }
    ...
    public Identifier getBlockID() {
        return new Identifier(shutterSettings.getBlockId() + "_open");
    }
